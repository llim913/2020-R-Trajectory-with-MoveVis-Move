...
#iris <- iris[-1,]
iris <- subset(df, select = -c(X1,stype,RECV_DT,ShipName,AREA_ID ,TRGT_SENSOR_KIND,MSG_ID,LOC_ACCRCY))
head(iris)
...
iris_mat
### 모형 설계행렬
iris.training <- iris_mat[ind==1, 1:5]
iris.test <- iris_mat[ind==2, 1:5]

### 모형 예측변수
iris.trainingtarget <- iris_mat[ind==1, 6]
iris.testtarget <- iris_mat[ind==2, 6]
...
model %>% 
  layer_dense(units = 8, activation = 'relu', input_shape = c(5)) %>% #4개가 들어가도록 설정 , relu 는 활성화 함수
  layer_dropout(rate = 0.5) %>%   # 모형 단순화
  layer_dense(units = 5, activation = 'softmax') # 입력계층 하나 출력계층 하나, softmax는 는 활성화 함수
